...
import { ArrayField, Box, Field, ObjectField, PointerField, PrimitiveField, Vertex } from "box"
import { createElement, JsxValue, replaceChildren } from "jsx"
						...
						<span className="name">{field.fieldName}</span>
						{
							field.accept<JsxValue>({
								visitPrimitiveField: (field: PrimitiveField): JsxValue => (
									<div className="value">
										<span>{`${field.getValue()}`}</span> <span className="pointer">{incoming(field)}</span>
									</div>
								),
								...
								visitArrayField: (field: ArrayField): JsxValue => (<span classNam="value">N:{field.size()}</span>),
								visitField: (field: Field): JsxValue => (
									<div className="value"><span>⌾</span> <span className="pointer">{incoming(field)}</span></div>)
							})
						}
					</div>
				)
