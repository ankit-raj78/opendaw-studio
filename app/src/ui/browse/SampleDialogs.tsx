	...
	export const missingSampleDialog = async (service: StudioService, uuid: UUID.Format, name: string): Promise<AudioSample> => {
		...
		const dialog: HTMLDialogElement = (
			<Dialog headline="Missing Sample"
								...
								onClick: async handler => {
									...
									if (isDefined(file)) {
										const {
											...
											service.importSample({ uuid, name: file.name, arrayBuffer: await file.arrayBuffer() }))
										if (status === "resolved") {
											handler.close()
											resolve(sample)
										}
				...
				<div
					style={{ padding: "1em 0", display: "grid", gridTemplateColumns: "auto 1fr", columnGap: "1em" }}>{name}</div>
			</Dialog>
		)
